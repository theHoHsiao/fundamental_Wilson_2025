...
def plot(data):
    ...
    for ax, ch in zip([data_axes], ["v"]):
        ...
        ax.set_ylabel(r"$ \Delta \hat{m}_{\mathrm{" + ch_tag(ch) + "}} \mathrm{(smear - gevp)} $  at  "+ r"$\beta$")
        ...
        for beta, colour, marker in beta_iterator(betas):
            ...
            for datum in data:
                ...
                if "smear_f_ps_mass_samples" not in datum:
                    continue
                
                X = ( datum["w0_samples"] * datum["smear_f_ps_mass_samples"]) ** 2
                Y = ( datum["w0_samples"] * datum[f"smear_f_{ch}_mass_samples"]) 

                X2 = ( datum["w0_samples"] * datum["gevp_f_ps_E0_mass_samples"]) ** 2
                Y2 = beta + ( datum["w0_samples"] * datum[f"gevp_f_{ch}_E0_mass_samples"]) - ( datum["w0_samples"] * datum[f"smear_f_{ch}_mass_samples"]) 
