...
def plot(data):
    ...
    for ax, ch in zip([data_axes[0], data_axes[1]], ["ps","v"]):
        ...
        ax.set_ylabel(r"$af_{\mathrm{" + ch_tag(ch) + "}}^2$")
        ...
        for beta, colour, marker in beta_iterator(betas):
            ...
            for datum in data:
                ...
                if "f_ps_decay_constant_samples" not in datum:
                    continue

                X = (  datum["gevp_f_ps_E0_mass_samples"]) ** 2
                Y = (  datum[f"f_{ch}_decay_constant_samples"]) ** 2
...
def main():
    ...
    data_fig = plot(data)
    save_or_show(data_fig, args.plot_file)
